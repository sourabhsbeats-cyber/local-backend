...
from store_admin.helpers import name_validator, getUserName, name_validator_none, reference_validator, date_validator, \
    zip_validator, \
    get_prep_label, format_tax_percentage, parse_date_or_none, safe_int, to_int_or_none, generateTrackingLink
...
from store_admin.models.po_models.po_models import PurchaseOrder, POStatus, PurchaseOrderItem, \
    ...
    VendorPaymentAllocations, VendorCredits, VendorLedger, POReceiveStatus, \
...
def save_po_receive_split_complete(request):
    ...
    for item in po_order_items:
        if item.ordered_qty != item.received_qty:
            return JsonResponse({"status": False, "error": "This has pending receive, Can not complete this order"},
                                status=404)
...
def all_shipments(request):
    ...
    po_shipments = PurchaseOrderShipping.objects.annotate(
        po_number=Subquery(po_sub.values("po_number")[:1]),
        vendor_po=Subquery(vendor_po_sub.values("vendor_po_number")[:1]),
        vendor_po_order_no=Subquery(vendor_po_sub.values("order_number")[:1]),
    ...
    if search:
        po_shipments = po_shipments.filter(
            Q(vendor_po__icontains=search) |
            Q(vendor_po_order_no__icontains=search)
        )
    ...
    if shipping_status not in [None, "", "all"]:
        po_shipments = po_shipments.filter(status=shipping_status)

    # ---------------------------------
    # SHIPPING DATE FILTER
    # ---------------------------------
    shipping_date_from = request.GET.get("shipping_date_from")
    shipping_date_to = request.GET.get("shipping_date_to")

    shipping_date_from = request.GET.get("shipping_date_from")
    shipping_date_to = request.GET.get("shipping_date_to")

    if shipping_date_from:
        po_shipments = po_shipments.filter(shipping_date__gte=shipping_date_from)

    if shipping_date_to:
        po_shipments = po_shipments.filter(shipping_date__lte=shipping_date_to)

    # ---------------------------------
    # SUMMARY
    # ---------------------------------
    total_count = po_shipments.count()

    status_counts = po_shipments.values("status").annotate(
        count=Count("status")
    )

    summary = {
        "total": {"count": total_count, "amount": 0},
    ...
    for item in status_counts:
        ...
        if status == 0:
            ...
            summary["shipped"]["count"] = count
        elif status == 2:
            summary["delivered"]["count"] = count
        elif status == 3:
            summary["cancelled"]["count"] = count
        elif status == 4:
            summary["returned"]["count"] = count
    ...
    for po_shipment in po_shipments:

        carrier_name = None
        tracking_number = None
        tracking_link = None

        if po_shipment.provider:
            tracking_number = po_shipment.tracking_number
            carrier_name, tracking_link = generateTrackingLink(
                po_shipment.provider,
                po_shipment.tracking_number
            )

        data.append({
            "vendor_po": po_shipment.vendor_po,
            "vendor_po_order_no": po_shipment.vendor_po_order_no,
            "vendor_po_order_date": po_shipment.vendor_po_order_date,
            "tracking_link": tracking_link,
            "tracking_number": tracking_number,
            "carrier_name": carrier_name,
            "status": po_shipment.status,
            "shipping_date": po_shipment.shipping_date,
            "po_number": po_shipment.po_number,
        })
